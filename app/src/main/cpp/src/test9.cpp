...
int GetH264Stream(const char *video_mp4, const char *video_h264);

extern "C"
JNIEXPORT void JNICALL
Java_com_xxx_media_Media_test9_1extract_1h264(JNIEnv *env, jclass type, jstring video_mp4_,
                                              jstring video_h264_) {
    const char *video_mp4 = env->GetStringUTFChars(video_mp4_, 0);
    const char *video_h264 = env->GetStringUTFChars(video_h264_, 0);

    int ret = GetH264Stream(video_mp4, video_h264);

    LOGI("GetH264Stream ret: %d", ret);

    env->ReleaseStringUTFChars(video_mp4_, video_mp4);
    env->ReleaseStringUTFChars(video_h264_, video_h264);
}
...
int GetH264Stream(const char *video_mp4, const char *video_h264) {
    ...
    fp = fopen(video_h264, "wb+");

    const char *InputFileName = video_mp4;
    ...
    avformat_alloc_output_context2(&oc, NULL, "h264", video_h264);
    ...
    if (!(ofmt->flags & AVFMT_NOFILE)) {
        ret = avio_open(&oc->pb, video_h264, AVIO_FLAG_WRITE);
        if (ret < 0) {
            printf("Could not open output file '%s'", video_h264);
