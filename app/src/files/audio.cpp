...
Java_com_xxx_media_Media_playSound(JNIEnv *env, jclass jclazz, jobject instance, jstring input_) {
    ...
    // 得到SwrContext ，进行重采样
    // 具体参考http://blog.csdn.net/jammg/article/details/52688506
    SwrContext *swrContext = swr_alloc();
    // 缓存区
    uint8_t *out_buffer = (uint8_t *) av_malloc(sample_rate * 2);
    // 输出的声道布局（立体声）
    ...
    // 反射调用createAudio
    env->CallVoidMethod(instance, createAudio, sample_rate, out_channer_nb);
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        if (packet->stream_index == audio_stream_idx) {
            ...
            if (got_frame) {
                swr_convert(swrContext, &out_buffer, sample_rate * 2, (const uint8_t **) frame->data,
                            frame->nb_samples);
                // 缓冲区的大小
